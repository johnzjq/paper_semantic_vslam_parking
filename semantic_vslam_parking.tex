...
In this paper, we proposed a noval solution of realtime indoor localization for autonomouse driving in parking lots.
High level landmarks, the parking spaces, are extracted and enriched with labaling to avoid the instablity of low-level visual features.
We then proposed a robust method for detecting incorrect data associations between parking spaces and further extended traditional optimization framework by dynamically evaluating and eliminating suboptimal data associations.
Limited visual Tags are also introduced for corners where no parking space presents.
As a result, a map of a parking lot can be established fully automatically and robustly.
We experimented the performance of realtime localization based on the semantic map using our automonouse driving platform TiEV and the average accuracy of 0.3m tracking tracing can be achieve at a speed of 10kph.
\end{abstract}
...
\IEEEPARstart{A}{utonomous} driving has made great progress in recent years, breakthrough has been made in several harsh fields, including obstacle detection, real-time motion planning and high preceision localization (mostly based on differential GNSS).
...
One critical reason is the lack of robust high precision localization mean in GNSS forbidden area such as indoor parking lots etc.
Traditional indoor localization methods require pre-equipmented sensors, such as WiFi, bluetooth or UWB. 
Wireless signal decays while user’s distance to signal sources increases, so a significant number of stations are needed for stability, despite of their relative low precision\citep{WIFI bluetooth UWB}. 
...
However, these range based representation is of high data volume and is vulnerable to dynamic scenes due to the lack of abilities of scene description.
As a result, visual SLAM built on low-cost cameras became one of the most favorable localization method.

%

Visual SLAM (VSLAM) is known to be effective in texture-rich environment\citep{ORB}.
Nevertheless, they can easily fail in monotone textured scene such an indoor parking lot.
\citet{VW paking} adopted sparse feature point based SLAM method with panorama images.
But sparse feature extracted with ?? can be unstable when the ground floor is stained with tire markings or water spots.
...
The direct methods estimate camera poses directly based on photometric error derived from whole images, thus are more robust than sparse methods in ?? area \citep{}.
However, they often require high frame rate and are susceptible to global illumination change, which restrict their usage in dimmed unevenly illuminated indoor space\citep{}. 
Moreover, the re-localization based on previously build dense map is not trivial (to verify).
