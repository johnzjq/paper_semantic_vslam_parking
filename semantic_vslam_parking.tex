...
\COMMENT{John: the link between these two line are not clear; Btw, how can a dense method realized re-localization? is it trivial or is it a shortcoming for these methods?}
...
Fiducial markers are introduced as an aid for the constancy of localization since few parking lot is detected near the entrances and exits \COMMENT{John:but we put the markers in the inner path of two rings?}. 
...
\COMMENT{John: ADD how to parameterize the parking slot, if we planned to optimize the shape of a parking slot? }
...
Due to fallible detection of parking spaces and their ids with low-quality surround vising images, ambiguities will be presented during data association.
...
% In this section, a map of parking spaces is built by fusing all the observation datum collected in the former sections. 
...
In our method all the observations of parking spaces and fiducial markers are treated as edges in the graph. 
% Observations from vehicle to tag or vehicle are presented by G2O standard edges. 
However, ambiguity raised from observations of parking spaces as well as parking ids lead to erroneous data associations and generate fatal residual error in the optimization. 
