...
In this paper, we proposed a novel solution of real-time indoor localization for autonomous driving in parking lots.
High level landmarks, the parking spaces, are extracted and enriched with labels to avoid the instability of low-level visual features.
We then proposed a robust method for detecting incorrect data associations between parking spaces and further extended traditional optimization framework by dynamically eliminating suboptimal data associations.
Limited visual fiducial Tags are also introduced for corners where no parking space presents.
...
\cite{Houben:2015hq} detects parking lots from the (fixed?) top view image, and use detected lots to estimate camera poses (is this a slam?).
...
A top-view image is thus fused from the surround-view inputs after intrinsic and extrinsic calibration, as shown in \Reffig{fig:1}. 
In the top-view image, who indicates ground textures, parking lots are detected. 
The monocular camera is installed at the left of the real-view mirror to capture front scenes. 
The change of steering wheel angle, as well as the vehicle speed and heading direction collected by IMU are also used for stability.
...
%do we still need fiducial markers to indicate pillars?
%TODO
