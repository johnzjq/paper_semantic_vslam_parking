...
Moreover, the re-localization based on pre-built dense map is not trivial since most direct methods are more like visual odometries rather than slam systems.
%Moreover, the performances of direct SLAM systems also depend on a good map initialization.\citep{Younes2016A}\citep{Engel2017Direct}
...
In the practice, we find that loop closure using only parking slots always fails, so visual markers are also placed where there is a high revisiting-rate.
